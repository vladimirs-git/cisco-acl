"""AddrGroup. Address items configured under "object-group network" (ios) or
...
class AddrGroup(Base, Group):
    ...
    def __init__(self, line: str = "", **kwargs):
        """AddrGroup
        :param str line: address-group config line
        :param str platform: Platform: "ios", "nxos" (default "ios")

        Helpers
        :param note: Object description
        :param str indent: Address lines indentation (default "  ")

        Alternate way to get `name` and `items`, if `line` absent
        :param str name: Address group name (default from `line`)
    ...
    def items(self, items: LUSAddressAg) -> None:
        if isinstance(items, (str, AddressAg)):
            items = [items]
    ...
    def copy(self) -> AddrGroup:
        """Copies the self object"""
        kwargs = self.data()
        return AddrGroup(**kwargs)
    ...
    def ipnets(self) -> LIpNet:
        """Address group IPv4Networks
        return: List of IpNetwork
        :raises ValueError: If one of the address is not contiguous wildcard
    ...
    def resequence(self, start: int = 10, step: int = 10, **kwargs) -> int:
        """Changes sequence numbers for addresses
    ...
    def wildcards(self) -> LStr:
        """Address group wildcards
        :return: Subnets with wildcard mask

        :example:
            self.items: [AddressAd("10.0.0.0/30"),
                         AddressAd("object-group ip address NAME")]  # 10.1.1.0/30
            return: [IpNetwork("10.0.0.0 0.0.0.3"), IpNetwork("10.1.1.0 0.0.0.3")]
        """
        wildcards: LStr = []
        for address in self._items:
