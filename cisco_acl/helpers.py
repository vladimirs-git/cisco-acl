...
from cisco_acl.types_ import DStr, LStr, StrInt, LInt, OInt, SInt, T2Str, T3Str, DInt, SStr
...
def is_contiguous_wildcard(wildcard: str) -> bool:
    ...
    if sum_ := sum([octets[0] * 256 ** 3,
                    octets[1] * 256 ** 2,
                    octets[2] * 256,
                    octets[3]]):
        sum_s = "{0:b}".format(sum_)
        bits: SStr = set(sum_s)
        if "0" in bits:
            return False
    return True
