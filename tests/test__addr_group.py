...
A_WILD_3_3 = AddressAg(WILD_NC3, platform="nxos")
A_WILD_252 = AddressAg(WILD_NC252, platform="nxos", max_ncwb=30)
...
class Test(Helpers):
    ...
    def test_valid__lt__(self):
        ...
        ag_pref = AddrGroup(platform="nxos", name="NAME", items=[PREFIX30])
    ...
    def test_valid__items(self):
        ...
        for items, req, in [
            ...
            obj = AddrGroup(name="NAME", items=items)
    ...
    def test_valid__data(self):
        ...
        for kwargs, req_d in [
            ...
            diff = list(dictdiffer.diff(first=result, second=req_d))
            self.assertEqual(diff, UUID_R2, msg=f"{kwargs=}")
    ...
    def test_valid__wildcards(self):
        """AddrGroup.wildcards()"""
        pref30 = AddressAg(SUBNET30)
        host = AddressAg(HOST)
        for items, req in [
            ([pref30, host], [WILD30, WILD32]),
            ([pref30, pref30], [WILD30, WILD30]),
        ]:
            obj = AddrGroup(name="NAME", items=items)
